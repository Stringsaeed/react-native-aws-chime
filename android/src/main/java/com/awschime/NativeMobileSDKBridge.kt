...
class NativeMobileSDKBridge(
  ...
  fun startMeeting(meetingInfo: ReadableMap, attendeeInfo: ReadableMap) {
    ...
    currentActivity?.let { activity ->
      ...
      try {
        ...
        eventEmitter.sendReactNativeEvent(
          RN_EVENT_ERROR,
          "Error starting the meeting session: ${exception.localizedMessage}"
        )
        return
      }
    }
  ...
  private fun createSessionConfiguration(
    meetingInfo: ReadableMap,
    attendeeInfo: ReadableMap
    ...
    return when (requestCode) {
      WEBRTC_PERMISSION_REQUEST_CODE -> {
        ...
        if (isMissingPermission) {
          eventEmitter.sendReactNativeEvent(
            RN_EVENT_ERROR,
            "Unable to start meeting as permissions are not granted"
          )
          false
        } else {
          startAudioVideo()
