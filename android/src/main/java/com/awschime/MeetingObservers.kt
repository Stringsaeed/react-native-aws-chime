...
import com.awschime.RNEventEmitter.Companion.RN_EVENT_ATTENDEES_JOIN
import com.awschime.RNEventEmitter.Companion.RN_EVENT_ATTENDEES_LEAVE
import com.awschime.RNEventEmitter.Companion.RN_EVENT_ATTENDEES_MUTE
import com.awschime.RNEventEmitter.Companion.RN_EVENT_ATTENDEES_UNMUTE
import com.awschime.RNEventEmitter.Companion.RN_EVENT_VIDEO_TILE_ADD
import com.awschime.RNEventEmitter.Companion.RN_EVENT_VIDEO_TILE_REMOVE
...
class MeetingObservers(private val eventEmitter: RNEventEmitter) : RealtimeObserver,
  ...
  override fun onAttendeesJoined(attendeeInfo: Array<AttendeeInfo>) {
    attendeeInfo.forEach {
      logger.info(
        TAG,
        "Received attendee join event for attendee: ${it.attendeeId} with externalUserId ${it.externalUserId}"
      )
  ...
  override fun onAttendeesLeft(attendeeInfo: Array<AttendeeInfo>) {
    attendeeInfo.forEach {
      logger.info(
        TAG,
        "Received attendee leave event for attendee ${it.attendeeId} with externalUserId ${it.externalUserId}"
      )
  ...
  override fun onAttendeesDropped(attendeeInfo: Array<AttendeeInfo>) {
    attendeeInfo.forEach {
      logger.info(
        TAG,
        "Received attendee drop event for attendee ${it.attendeeId} with externalUserId ${it.externalUserId}"
      )
      eventEmitter.sendAttendeeUpdateEvent(RN_EVENT_ATTENDEES_LEAVE, it)
    }
  }
  ...
  override fun onAttendeesMuted(attendeeInfo: Array<AttendeeInfo>) {
    attendeeInfo.forEach {
      logger.info(
        TAG,
        "Received attendee mute event for attendee ${it.attendeeId} with externalUserId ${it.externalUserId}"
      )
  ...
  override fun onAttendeesUnmuted(attendeeInfo: Array<AttendeeInfo>) {
    attendeeInfo.forEach {
      logger.info(
        TAG,
        "Received attendee unmute event for attendee ${it.attendeeId} with externalUserId ${it.externalUserId}"
      )
